...
from app.models.ocr_model import is_model_loaded, load_model, recognize
from app.proto import ocr_pb2, ocr_pb2_grpc
...
class OCRServicer(ocr_pb2_grpc.OCRServiceServicer):
    ...
    def RecognizeText(self, request, context):
        effective_lang = request.language or OCR_LANGUAGE
        _remaining = context.time_remaining()
        if _remaining is not None and _remaining <= 0:
            context.set_code(grpc.StatusCode.DEADLINE_EXCEEDED)
            context.set_details("Deadline exceeded before inference")
            return ocr_pb2.OCRResponse(service_level="FALLBACK", language_used=effective_lang)
        ...
        try:
            ...
            blocks = [
                ocr_pb2.TextBlock(
                    ...
                    x2=b.get("x2", 0.0), y2=b.get("y2", 0.0),
                )
            ...
            logger.warning(f"RecognizeText invalid input: {e}")
            ...
            logger.warning(f"RecognizeText overloaded: {e}")
            INFERENCE_COUNTER.labels(status="error").inc()
            INFERENCE_LATENCY.labels(status="error").observe(time.perf_counter() - start)
            ...
            logger.error(f"RecognizeText error: {e}")
            INFERENCE_COUNTER.labels(status="error").inc()
            INFERENCE_LATENCY.labels(status="error").observe(time.perf_counter() - start)
    ...
    def HealthCheck(self, request, context):
        s = get_health_status("paddleocr", is_loaded=is_model_loaded())
...
def start_grpc_server(model_ready_event: multiprocessing.Event):
    ...
    if not GRPC_AUTH_TOKEN:
        logger.warning(
            "GRPC_AUTH_TOKEN tidak dikonfigurasi — OCR Worker berjalan TANPA autentikasi. "
    ...
    logger.info(f"OCR gRPC server started on port {GRPC_PORT} ({scheme})")
